...
class Mas(object):
	def __init__(self, source):
		...
		# self.target = target
		self.reg_params = {}
		self.reg_params_intersect = {}

def init_reg_params(model, device, selected_blocks = [], selected_layers = [], linear1 = False, linear2 = False, last_conv = False):

	model.source = peel_model(model.source)
	reg_params={}
	block_names=[]
	...
	for name, param in model.source.named_parameters():
	...
	model.reg_params_intersect = reg_params_intersect


	return model, block_names
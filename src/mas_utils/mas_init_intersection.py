...
def init_no_intersection(Gen, Gen_block, Gen_reg_params, Gen_reg_params_intersect, \
    ...
    optimizer_ft_gen = omega_update(Gen.parameters())
    optimizer_ft_dis = omega_update(Dis.parameters())
    ...
    misc.plot_img_canvas(images=fake_images.detach().cpu(),
                        ...
                        "figures/{run_name}/test_selectG_topk_ratio_{selectG_topk_ratio}_selectG_blocks_{selectG_blocks}_selectG_layers_{selectG_layers}_selectD_topk_ratio_{selectD_topk_ratio}_selectD_blocks_{selectD_blocks}_selectD_layers_{selectG_layers}.png".\
                        format(run_name = run_name, selectG_topk_ratio = cfgs.RUN.selectG_topk_ratio, selectG_blocks = cfgs.RUN.selectG_blocks, selectG_layers = cfgs.RUN.selectG_layers, \
                               selectD_topk_ratio = cfgs.RUN.selectD_topk_ratio, selectD_blocks = cfgs.RUN.selectD_blocks, selectD_layers = cfgs.RUN.selectD_layers)),
                        num_cols=8,
                        logger=logger,
    ...
    for step_index in range(50000//cfgs.OPTIMIZATION.batch_size):
        ...
        optimizer_ft_gen.step(Gen_reg_params, step_index, cfgs.OPTIMIZATION.batch_size, local_rank)

        if dis_change == True:
            optimizer_ft_dis.step(Dis_reg_params, step_index, cfgs.OPTIMIZATION.batch_size, local_rank)
    ...
    sd_gen = copy.deepcopy(Gen.state_dict())
    sd_dis = copy.deepcopy(Dis.state_dict())
    ...
    if change == True:
        for name, _ in Gen.named_parameters():
            if name not in Gen_block:
                ...
                print("topk_ratio:{percent}%".format(percent = 100/cfgs.RUN.selectG_topk_ratio))
                size = list(value_clone.size())[0]

                idx = torch.topk(value_clone, k= size//cfgs.RUN.selectG_topk_ratio)
                ...
                idx_sel = torch.from_numpy(idx_sel).to(local_rank)

                print("idx_sel",idx_sel)
                print("idx_sel",idx_sel.size())
                ...
                # print(param_clone)
                # print(w)
                # print("param_clone",param_clone.size())
                # print("w",w.size())

                # param_clone = copy.deepcopy(sd_gen[name])
                param_clone = param_clone.reshape(-1)
                w = w.reshape(-1)

                # print("param_clone reshape",param_clone[idx_sel])

                param_clone[idx_sel] = w[idx_sel]
                # print("param_clone reshape",param_clone[idx_sel])
                param_clone = torch.reshape(param_clone, original_size)
                sd_gen[name] = param_clone

        if dis_change == True:
            print("============================================Dis reinit============================================")   
            for name, _ in Dis.named_parameters():
                if name not in Dis_block:
                    for value in Dis_reg_params.values():
                    ...
                    print("================before intersection================")
                    print("value_clone",value_clone.shape)
                    print("topk_ratio:{percent}%".format(percent = 100/cfgs.RUN.selectD_topk_ratio))
                    size = list(value_clone.size())[0]

                    idx = torch.topk(value_clone, k= size//cfgs.RUN.selectD_topk_ratio)
                    ...
                    print(name)
                    print(sd_dis[name].size())
                    param_clone = sd_dis[name].clone()
                    param_clone = param_clone.reshape(-1)
                    w = w.reshape(-1)

                    param_clone[idx_sel] = w[idx_sel]
                    print(param_clone.size(), original_size)
                    param_clone = torch.reshape(param_clone, original_size)
                    sd_dis[name] = param_clone
    ...
    misc.plot_img_canvas(images=fake_images.detach().cpu(),
                        save_path=join(cfgs.RUN.save_dir,
                        "figures/{run_name}/MAS_no_intersection_selectG_topk_ratio_{selectG_topk_ratio}_selectG_blocks_{selectG_blocks}_selectG_layers_{selectG_layers}_selectD_topk_ratio_{selectD_topk_ratio}_selectD_blocks_{selectD_blocks}_selectD_layers_{selectG_layers}.png".\
                        format(run_name = run_name, selectG_topk_ratio = cfgs.RUN.selectG_topk_ratio, selectG_blocks = cfgs.RUN.selectG_blocks, selectG_layers = cfgs.RUN.selectG_layers, \
                            selectD_topk_ratio = cfgs.RUN.selectD_topk_ratio, selectD_blocks = cfgs.RUN.selectD_blocks, selectD_layers = cfgs.RUN.selectD_layers)),
                        num_cols=8,
                        logger=logger,

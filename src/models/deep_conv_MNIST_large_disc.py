    ...
    def forward(self, z, label, shared_label=None, eval=False):
        ...
        with torch.cuda.amp.autocast() if self.mixed_precision and not eval else misc.dummy_context_mgr() as mp:
            ...
            if len(affine_list) > 0:
                ...
                affines = None
            # print("=======================affines check2=====================")
            # print(affines)
            # print("affines",affines.size())
...
class Discriminator(nn.Module):
    def __init__(self, img_size, d_conv_dim, apply_d_sn, apply_attn, attn_d_loc, d_cond_mtd, aux_cls_type, d_embed_dim, normalize_d_embed,
        ...
        self.activation = MODULES.d_act_fn
        #change
        self.conv1 = MODULES.d_conv2d(in_channels=128, out_channels=256, kernel_size=3, stride=1, padding=1)
        ...
        if self.d_cond_mtd == "MH":
            self.linear1 = MODULES.d_linear(in_features=256, out_features=1 + num_classes, bias=True)
        elif self.d_cond_mtd == "MD":
            self.linear1 = MODULES.d_linear(in_features=256, out_features=num_classes, bias=True)
        else:
            self.linear1 = MODULES.d_linear(in_features=256, out_features=1, bias=True)
        ...
        if self.d_cond_mtd == "AC":
            self.linear2 = MODULES.d_linear(in_features=256, out_features=num_classes, bias=False)
        elif self.d_cond_mtd == "PD":
            self.embedding = MODULES.d_embedding(num_classes, 256)
        elif self.d_cond_mtd in ["2C", "D2DCE"]:
            self.linear2 = MODULES.d_linear(in_features=256, out_features=d_embed_dim, bias=True)
        ...
        if self.aux_cls_type == "TAC":
            if self.d_cond_mtd == "AC":
                self.linear_mi = MODULES.d_linear(in_features=256, out_features=num_classes, bias=False)
            elif self.d_cond_mtd in ["2C", "D2DCE"]:
                self.linear_mi = MODULES.d_linear(in_features=256, out_features=d_embed_dim, bias=True)
        ...
        if self.MODEL.info_type in ["discrete", "both"]:
            ...
            self.info_discrete_linear = MODULES.d_linear(in_features=256, out_features=out_features, bias=False)
        if self.MODEL.info_type in ["continuous", "both"]:
            out_features = self.MODEL.info_num_conti_c
            self.info_conti_mu_linear = MODULES.d_linear(in_features=256, out_features=out_features, bias=False)
            self.info_conti_var_linear = MODULES.d_linear(in_features=256, out_features=out_features, bias=False)

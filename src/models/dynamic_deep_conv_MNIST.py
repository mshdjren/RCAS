...
class Generator(nn.Module):
    def __init__(self, z_dim, g_shared_dim, img_size, g_conv_dim, apply_attn, attn_g_loc, g_cond_mtd, num_classes, g_init, g_depth,
        ...
        #change
        self.in_dims = [256, 128]
        self.out_dims = [128, 64]
        ...
        #change
        self.linear0 = MODULES.g_linear(in_features=self.z_dim, out_features=self.in_dims[0]*7*7, bias=True)
        ...
        #change
        self.conv3 = MODULES.g_conv2d(in_channels=self.out_dims[-1], out_channels=1, kernel_size=3, stride=1, padding=1)
        self.tanh = nn.Tanh()
    ...
    def forward(self, z, label, shared_label=None, eval=False):
        ...
        with torch.cuda.amp.autocast() if self.mixed_precision and not eval else misc.dummy_context_mgr() as mp:
            ...
            act = self.linear0(z)
            #change
            act = act.view(-1, self.in_dims[0], 7, 7)
...
class Discriminator(nn.Module):
    def __init__(self, img_size, d_conv_dim, apply_d_sn, apply_attn, attn_d_loc, d_cond_mtd, aux_cls_type, d_embed_dim, normalize_d_embed,
                 num_classes, d_init, d_depth, mixed_precision, MODULES, MODEL, dynamic_kwargs = {}, keepd = 0):
        super(Discriminator, self).__init__()
        self.in_dims = [1] + [64]
        self.out_dims = [64, 128]
        ...
        for index in range(len(self.in_dims)):
            self.blocks += [[
                DiscBlock(in_channels=self.in_dims[index], out_channels=self.out_dims[index], apply_d_sn=self.apply_d_sn, MODULES=MODULES, dynamic_kwargs = dynamic_kwargs, keepd=keepd,
        ...
        self.conv1 = DiscriminatorEpilogue(in_channels=self.out_dims[-1], out_channels=self.out_dims[-1]*2, apply_d_sn=self.apply_d_sn, MODULES=MODULES, keepd=keepd,
                          first_layer_idx = cur_layer_idx, dynamic_kwargs = dynamic_kwargs)
        cur_layer_idx += 1 

                                    
        if not self.apply_d_sn:
            self.bn1 = MODULES.d_bn(in_features=256)
       
       # linear layer for adversarial training
        if self.d_cond_mtd == "MH":
            self.linear1 = FullyConnectedLayer(256, 1 + self.num_classes, bias=True)
        elif self.d_cond_mtd == "MD":
            self.linear1 = FullyConnectedLayer(256, self.num_classes, bias=True)
        else:
            self.linear1 = FullyConnectedLayer(256, 1, bias=True)
        ...
        if self.d_cond_mtd == "AC":
            self.linear2 = FullyConnectedLayer(256, num_classes, bias=False)
        elif self.d_cond_mtd == "PD":
            self.linear2 = FullyConnectedLayer(256, self.cmap_dim, bias=True)
        elif self.d_cond_mtd in ["2C", "D2DCE"]:
            self.linear2 = FullyConnectedLayer(256, d_embed_dim, bias=True)
        ...
        if self.aux_cls_type == "TAC":
            if self.d_cond_mtd == "AC":
                self.linear_mi = FullyConnectedLayer(256, num_classes, bias=False)
            elif self.d_cond_mtd in ["2C", "D2DCE"]:
                self.linear_mi = FullyConnectedLayer(256, d_embed_dim, bias=True)
                self.embedding = MODULES.d_embedding(num_classes, d_embed_dim)
            else:
                raise NotImplementedError
        ...
        if self.MODEL.info_type in ["discrete", "both"]:
            ...
            self.info_discrete_linear = FullyConnectedLayer(in_features=256, out_features=out_features, bias=False)
        if self.MODEL.info_type in ["continuous", "both"]:
            out_features = self.MODEL.info_num_conti_c
            self.info_conti_mu_linear = FullyConnectedLayer(in_features=256, out_features=out_features, bias=False)
            self.info_conti_var_linear = FullyConnectedLayer(in_features=256, out_features=out_features, bias=False)
